...
class Bin:
    ...
    def write_array_byte(self, ba):
        ...
        if isinstance(ba, types.StringType): # byte sequences often come as strings
            self.__write_string(ba)
        else:
...
def pack(serializable):
    ...
    global TYPE_I, TYPE_Y, TYPE_B, TYPE_S, TYPE_AS, TYPE_AI, TYPE_AY
...
def unpack(serializable, bytes):
    ...
    @param serializable: the Serializable to apply the binary data to
    @param bytes: binary data (serialized Serializable)
    ...
    try:
        ...
        log.warning("binary data malformed (%s)" % e)
